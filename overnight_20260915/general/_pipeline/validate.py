"""Native high-sampling audit and final export for GENERAL; no fitted MTF surrogates."""
from pathlib import Path
import argparse,json,csv,atexit,hashlib
import numpy as np
...
for i in range(1,13):
 assert sys.LDE.GetSurfaceAt(i).Conic==0., 'GENERAL requires zero conic on every optical surface'
...
for fn in ['5.6','8','22']:
 ...
 assert np.all(np.isfinite(high)) and np.all(high>=0) and np.all(high<=1.00001), 'Invalid native MTF values'
 out[fn]=high.tolist();samples[fn]=size;convergence[fn]=float(np.max(abs(high-low)))
 print('GENERAL native audit',fn,'max shortfall',float(np.max(np.array(target['data'][fn])-high)),'sampling delta',convergence[fn],flush=True)
 ...
 title.Title='Super-Symmar XL 150 mm - GENERAL A6 reverse candidate - full 105 deg'
 title.Author='Reverse engineering study'
 title.Notes='GENERAL: A4/A6 only, A8 and higher zero; audited manufacturer digitization; full 105 degree field at all apertures; fixed clear apertures; automatic stop; common image plane. See merged optical and manufacturing report for numerical acceptance, limitations, geometry and glass substitutes.'
 p=R/f'SuperSymmarXL_150_GENERAL_f{fn.replace(".","p")}.zmx'
...
surfaces=[]
for i in range(1,13):
 ss=sys.LDE.GetSurfaceAt(i);surfaces.append({'surface':i,'radius_mm':ss.Radius,'thickness_mm':ss.Thickness,'glass':ss.Material,'clear_radius_mm':ss.SemiDiameter,'mechanical_radius_mm':ss.MechanicalSemiDiameter,'stop':bool(ss.IsStop),'conic':float(ss.Conic)})
coef=[sys.LDE.GetSurfaceAt(10).GetCellAt(i).DoubleValue for i in range(13,20)]
assert not any(coef[2:]), 'A8 and higher must be zero'
