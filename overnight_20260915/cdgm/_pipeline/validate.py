"""Native high-sampling audit and final export for CDGM; no fitted MTF surrogates."""
...
for i in range(1,13):
 assert sys.LDE.GetSurfaceAt(i).Conic==0., 'CDGM requires zero conic on every optical surface'
...
for fn in ['5.6','8','22']:
 ...
 print('CDGM native audit',fn,'max shortfall',float(np.max(np.array(target['data'][fn])-high)),'sampling delta',convergence[fn],flush=True)
 ...
 title.Title='Super-Symmar XL 150 mm - CDGM A6 reverse candidate - full 105 deg'
 title.Author='Reverse engineering study'
 title.Notes='CDGM: A4/A6 only, A8 and higher zero; audited manufacturer digitization; full 105 degree field at all apertures; fixed clear apertures; automatic stop; common image plane. See merged optical and manufacturing report for numerical acceptance, limitations, geometry and glass substitutes.'
 p=R/f'SuperSymmarXL_150_CDGM_f{fn.replace(".","p")}.zmx'
