"""Build the single CDGM report after final Zemax validation.
...
def main():
    ...
    if curves.get('sampling') != 512:
        raise ValueError('CDGM report plots require native FFT512 field_curves.json.')
    if v.get('sampling') != {'5.6': 1024, '8': 512, '22': 512}:
        raise ValueError('CDGM report quantitative audit requires FFT1024 at f/5.6 and FFT512 at f/8 and f/22.')
    ...
    if len(sur) != 12 or any('conic' not in s or float(s['conic']) != 0 for s in sur):
        raise ValueError('CDGM requires explicit zero conic on all twelve surfaces.')
    if float(v.get('asphere_conic', 0)) != 0:
        raise ValueError('CDGM top-level asphere_conic must also be zero.')
    signature = surface_signature(sur)
    if [s['surface'] for s in signature] != list(range(1, 13)):
        raise ValueError('CDGM surface signature must contain each surface 1 through 12 exactly once.')
    ...
    if len(sur) != 12 or coef.shape != (2,):
        raise ValueError('CDGM requires 12 surfaces and exactly two coefficients: A4, A6.')
    for key in ['asphere_A4_to_A12', 'asphere_A4_to_A16']:
        if key in v and np.any(np.asarray(v[key], dtype=float)[2:] != 0):
            raise ValueError(f'CDGM forbids nonzero A8 and higher: {key}')
    if 'asphere_powers' in v and list(v['asphere_powers']) != [4, 6]:
        raise ValueError('CDGM asphere_powers must be [4, 6].')
    ...
    for ax in axs:
        ...
        ax.grid(alpha=.13)
    axs[0].set(xlim=(-4, max(82, z[-1] + 4)), ylim=(-44, 44), title='CDGM optical section: full 105-degree field')
    ...
    md = ['# Super-Symmar XL 150 mm f/5.6 光学性能与制造可能性报告 - CDGM\n']
    story = []
    ...
    def footer(c, d):
        c.setSubject('CDGM source SHA256: ' + final_hash)
        c.setFont('SXL', 8)
        c.setFillColor(colors.HexColor('#77838c'))
        c.drawString(40, 26, 'Super-Symmar XL 150 mm | CDGM reverse candidate')
    ...
    story.append(Paragraph('光学性能与制造可能性报告<br/>Super-Symmar XL 150 mm f/5.6 - CDGM', styles['title']))
    section('方案与材料身份', '全 CDGM 材料优化方案' + '。本报告仅引用本方案冻结 best 的原生复算，不将其等同于旧 R4 材料或处方。材料替换并不自动带来性能提高或实测降本。')
    ...
    tab([['参数', 'CDGM 最终数值', '单位'], ['圆锥常数 k', str(sur[9].get('conic', v.get('asphere_conic', 0))), '无量纲'], *[[f'A{power}', f'{a:.12e}', f'mm^({1-power})'] for a, power in zip(coef, powers)]], [80, 290, 145])
    ...
    section('结构变化按最终数据统计', '以下变化由最终 CDGM 与 R2 的 validated.json 逐面比较得出，不假设本轮实际使用了哪组优化变量或限制。未列出的对应数值在 1e-8 mm 比较阈值内一致。S12 间隔变化表示共同像面的变化，不是镜片中心厚度。')
    tab([['面号', 'CDGM - R2'], *(changed or [['全部面', '处方表中的曲率、间隔、口径与玻璃未检出变化']])], [70, 445])
    ...
    if manufacturing is None:
        ...
        if not coefficients_match or not radius_match:
            section('制造指标与最终输入不一致', '现有 manufacturing_geometry_metrics.json 的系数或顶点曲率与本次 validated.json 不一致，不能作为本次最终处方的制造几何证明。必须重新运行 finalize_overnight.py --phase cdgm --prepare-only；本报告不引用其旧非球面数值。')
        else:
    ...
    section('复算数据与文件', 'overnight_20260915/cdgm/validated.json：本轮最高 A6 候选的最终高采样离散 MTF、处方与一阶数据。overnight_20260915/cdgm/field_curves.json：本轮完整视场模拟曲线。overnight_20260915/cdgm/target_optimization.json：本轮正式手册目标。revision2/validated.json：历史处方结构对照；revision2/drawing_constraints.json：结构图定标记录。revision3 中的高阶候选与 Huygens 初审仅属于历史资料，不代表本轮 A6 处方已复核。全部产出保存在 SuperSymmarXL 文件夹中。')
