"""Compute geometric manufacturing descriptors from validated CDGM data, no ZOS.
...
parser.add_argument('--input', type=Path, default=Path('D:\\_Camera\\LensRebuild\\SuperSymmarXL\\overnight_20260915\\cdgm\\validated.json'))
...
if len(v['surfaces']) != 12 or any('conic' not in s or float(s['conic']) != 0 for s in v['surfaces']):
    raise ValueError('CDGM manufacturing metrics require explicit zero conic on all twelve surfaces.')
...
if coeff.shape != (2,):
    raise ValueError('CDGM requires exactly two coefficients, A4 and A6.')
...
for i,s in sorted(sur.items()):
    ...
    changes.append({'surface':i,'glass':s['glass'],'stop':bool(s.get('stop',False)),
                    'R2_center_distance_mm':float(p['thickness_mm']), 'CDGM_center_distance_mm':float(s['thickness_mm']),
                    ...
                    'R2_clear_radius_mm':float(p['clear_radius_mm']),'CDGM_clear_radius_mm':float(s['clear_radius_mm']),
...
metrics = {'input':str(args.input.resolve()),'comparison_R2':str(args.r2.resolve()),
           ...
           'warnings': ['Nominal geometry only. No process precision, manufacturing cost, tolerance, yield or supplier qualification is inferred.',
                        ...
                        'All twelve conics must be explicitly stored as zero. CDGM is restricted to A4 and A6; nonzero A8 and higher are rejected. Low polynomial order does not prove historical or present manufacturing capability.',
...
fig.suptitle('CDGM nominal S10 geometry: designed shape, not a manufacturing tolerance')
