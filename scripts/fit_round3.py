...
x0=np.r_[json.loads((ROOT/'analysis'/'fit_round2.json').read_text())['x'],0.0,0.0]
...
def fun(x):
 ...
 return res
r=least_squares(fun,x0,bounds=bounds,diff_step=1e-3,x_scale=np.array([1,2,5,1,1]+[.01]*11+[5,5]),max_nfev=100,verbose=0,ftol=1e-7,xtol=1e-8)
apply(r.x)
