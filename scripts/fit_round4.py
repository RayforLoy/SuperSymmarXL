...
x0=np.r_[json.loads((ROOT/'analysis'/'fit_round3.json').read_text())['x']]

bounds=([-30,-100,-100,132,.4]+[-.05]*len(base_r)+[-100,-100],[30,100,100,140,5.66]+[.05]*len(base_r)+[100,100])
def apply(x):
...
def fun(x):
 ...
 res.extend([(efl-148.1)*.1,(x[3]-135.9)*.025]);res.extend(x[5:16]*.1)
 ...
 return res
steps=np.array([.0001,.001,.002,.001,.005]+[2e-6]*11+[.01,.01])
def jac(x):
 cols=[]
...
r=least_squares(fun,x0,bounds=bounds,jac=jac,x_scale=np.array([1,2,5,1,1]+[.01]*11+[5,5]),max_nfev=35,verbose=0,ftol=1e-7,xtol=1e-8)
apply(r.x)
