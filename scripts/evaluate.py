exec(open(__file__.replace('evaluate.py','probe_zos.py')).read().split("print('system'")[0])
...
start=time.time();a.ApplyAndWaitForCompletion();res=a.GetResults();res.GetTextFile(str(ROOT/'analysis'/'baseline_mtf.txt'))
...
sys.SaveAs(str(ROOT/'models'/'baseline_real_aim.zmx'))
