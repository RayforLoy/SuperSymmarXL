"""Compute geometric manufacturing descriptors from validated R3 data, no ZOS.
...
parser.add_argument('--input', type=Path, default=P/'revision3/validated.json')
...
coeff = np.asarray(v.get('asphere_A4_to_A16', v.get('asphere_A4_to_A12', [])), dtype=float)
if len(coeff) not in [5, 6, 7]:
    raise ValueError('Expected five to seven S10 coefficients, A4..A12/A14/A16.')
powers = np.arange(4, 4+2*len(coeff), 2)
...
for i,s in sorted(sur.items()):
    ...
    changes.append({'surface':i,'glass':s['glass'],'stop':bool(s.get('stop',False)),
                    'R2_center_distance_mm':float(p['thickness_mm']), 'R3_center_distance_mm':float(s['thickness_mm']),
                    ...
                    'R2_clear_radius_mm':float(p['clear_radius_mm']),'R3_clear_radius_mm':float(s['clear_radius_mm']),
...
metrics = {'input':str(args.input.resolve()),'comparison_R2':str(args.r2.resolve()),
           ...
           'warnings': ['Nominal geometry only. No process precision, manufacturing cost, tolerance, yield or supplier qualification is inferred.',
                        ...
                        'Conic defaults to zero if validated.json omits it. Coefficient order is inferred from the stored five to seven even coefficients starting at A4; optimizer variable restrictions are not inferred.',
...
fig.suptitle('R3 nominal S10 geometry: designed shape, not a manufacturing tolerance')
fig.tight_layout();fig.savefig(R/'asphere_profile.png',dpi=180);plt.close(fig)
