...
parser=argparse.ArgumentParser();parser.add_argument('--input',default='SuperSymmarXL_150_R4_f5p6.zmx');args=parser.parse_args()
...
R=ROOT/'revision4';sys.LoadFile(str(R/args.input),False)
...
for bi,hs in enumerate(batches):
 ...
 for fn in values:
  ...
  print('R4 curve batch',bi+1,fn,flush=True)
